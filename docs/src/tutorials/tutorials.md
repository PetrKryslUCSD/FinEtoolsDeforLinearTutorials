[Table of contents](https://petrkryslucsd.github.io/FinEtoolsDeforLinearTutorials.jl/dev/index.html)

# Tutorials

## Statics

- [Cook membrane, plane stress](Cook-plane-stress_tut.md)
- [Composite benchmark R0031-3](R0031-3-Composite-benchmark_tut.md)
- [Twisted beam, export to Abaqus](twisted_beam-export-to-abaqus_tut.md)

## Dynamics

- [Nearly incompressible cube vibration](unit_cube_modes_tut.md)
- [Nearly incompressible cube vibration, alternative models](unit_cube_modes_alt_tut.md)
- [13H benchmark, forced vibration](TEST13H_tut.md)
- [13H benchmark, forced vibration, parallel execution](TEST13H_par_tut.md)
- [13H benchmark, forced vibration, modal model](TEST13H_mod_tut.md)
- [FV32 benchmark, free vibration of trapezoidal membrane](FV32_tut.md)
- [Tracking transient deformation of a cantilever beam](bending_wave_Ray_tut.md)
- [Tracking transient deformation of a cantilever beam, lumped mass matrix](bending_wave_Ray_lumped_tut.md)
- [Tracking transient deformation of a cantilever beam, centered difference](bending_wave_Ray_expl_cd_tut.md)
- [Beam under on/off loading: transient response](beam_load_on_off_tut.md)
- [Suddenly-stopped bar: TW explicit](sudden_stop_expl_tw_tut.md)
- [Suddenly-stopped bar: CD explicit](sudden_stop_expl_cd_tut.md)